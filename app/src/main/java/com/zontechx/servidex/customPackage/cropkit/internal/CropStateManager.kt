...
import com.zontechx.servidex.customPackage.cropkit.util.Extensions.isInsideRect
import com.zontechx.servidex.customPackage.cropkit.util.GestureUtils
import com.zontechx.servidex.customPackage.cropkit.util.MathUtils
...
private fun moveCropRect(dragAmount: Offset) {
    ...
    val epsilon = 0.001f // Small tolerance to handle floating-point errors
    val maxLeft = if (imageRect.right - currentRect.width < imageRect.left)
        imageRect.left + epsilon
    else
        imageRect.right - currentRect.width
    val maxTop = if (imageRect.bottom - currentRect.height < imageRect.top)
        imageRect.top + epsilon
    else
        imageRect.bottom - currentRect.height

    val newLeft = (currentRect.left + dragAmount.x)
        .coerceIn(imageRect.left, maxLeft)
    val newTop = (currentRect.top + dragAmount.y)
        .coerceIn(imageRect.top, maxTop)
